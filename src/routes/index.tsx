    ...
    <main className="min-h-screen paper-grid">
      ...
      <section className="mx-auto grid max-w-6xl gap-10 px-5 pt-8 pb-16 lg:grid-cols-[1.1fr_1fr] lg:items-center lg:pt-16">
        <div className="animate-rise">
          ...
          <p className="mt-6 max-w-md text-base text-muted-foreground md:text-lg">
            Enter where you are and where you're going. TransitAI finds nearby bus stops, searches every bus combination
            and ranks the top 3 journeys — with clear step-by-step directions.
          </p>
          <ul className="mt-8 grid max-w-md grid-cols-3 gap-3 text-xs">
            <Feature icon={RouteIcon} title="Graph search" text="Dijkstra over stops, routes & transfers" />
            <Feature icon={GitBranch} title="5 preferences" text="Fastest, least walking, cheapest…" />
            <Feature icon={Database} title="Import-ready" text="CSV / database records" />
          </ul>
        </div>
      ...
      <section className="border-t bg-card/60">
        <div className="mx-auto grid max-w-6xl gap-8 px-5 py-12 md:grid-cols-3">
          <HowStep n="01" title="Find nearby stops" text="Both ends are geocoded, then several walkable stops around each are considered — not just the closest one." />
          ...
          <HowStep n="03" title="Rank & explain" text="Time, walking, transfers and fare are combined per your preference. Explanations only restate computed facts — the AI never invents buses or stops." />
        </div>
      </section>
